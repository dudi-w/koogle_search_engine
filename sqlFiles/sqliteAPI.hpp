...
public:
    ...
    int execute(std::string const& query) const;
    se::SqlResult executeQuery(std::string const& query) const;

// private:
    // int callback(void* data, int argc, char** argv, char** colNames);
private:
    sqlite3* m_db;
    // std::function m_callback;// Callback function for SELECT queries
};
